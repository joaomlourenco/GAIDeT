...
\begin{filecontents}{aidisclose-doc.bib}
...
\usepackage{biblatex}
\addbibresource{aidisclose-doc.bib}
...
Use the \verb|\GAIactivate{<key>}| command to check specific items in the taxonomy. A full list of available keys is provided in Section~\ref{sec:taxonomy-keys}.
...
To print the final declaration, use the \verb|\GAIrenderDeclaration| command at the desired location in your document.  For papers/articles, you may consider to it before or after the references.  For thesis/dissertations, please follow your University/School rules.

\begin{verbatim}
\GAIrenderDeclaration[<checklist_ncols>]{<Author Names>}
\end{verbatim}

\begin{itemize}
    \item \textbf{Optional Argument} (\verb|[<checklist_ncols>]|): The number of columns used while printing the checklist. The default is \texttt{3} columns.
    \item \textbf{Mandatory Argument}: A comma-separated list of author names submitting the declaration.
\end{itemize}


\begin{verbatim}
\GAIsetChecklistFontSize{<fontsize>}
\end{verbatim}

\begin{itemize}
    \item \textbf{Mandatory Argument}: The font size to use while printing the checklist. The default is \verb|\smaller[1]|, which means “\emph{slightly smaller}”.
\end{itemize}
...
\textbf{Example:}
\begin{verbatim}
\GAIrenderDeclaration[1]{Jane Doe, John Smith}
\end{verbatim}
\noindent prints the declaration for thwo authors with the Generative AI Checklist in a single column.

\section{Taxonomy Keys}
\label{sec:taxonomy-keys}
The following keys correspond to the specific tasks defined in the GAIDeT taxonomy~\cite{Suchikova:GAIDeT:2025}. Use these keys with the \verb|\GAIactivate{}| command.
...
\begin{GAIcomment*}
  This is a third additional comment!   It is straed and, thus, not get a number. also, this comment is longer and, hopefully, will spill to the next line!
\end{GAIcomment*}

\begin{GAIcomment}
  This is the third non-stared additional comment!
\end{GAIcomment}
