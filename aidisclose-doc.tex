...
\date{2025-12-30~(\gaiversion)}
\maketitle

\begin{abstract}
    The \texttt{aidisclose} package provides a standardized, transparent mechanism for declaring the use of Generative Artificial Intelligence (GAI) tools in academic, technical, and professional documents.
    It implements the \emph{GAIDeT (Generative AI Delegation Taxonomy)}~\cite{Suchikova:2025:GAIDeT} and automates the creation of disclosure statements and checklists.
...
\texttt{aidisclose} is designed to support emerging ethical, institutional, and publisher requirements concerning AI-assisted content creation.
...
\subsection{Options}
The package currently supports the following key-value options:
...
Use these keys with \latexinline|\GAIactivate{}|.
Derived from the GAIDeT taxonomy~\cite{Suchikova:2025:GAIDeT}.
...
Below is an example of a rendered declaration.
